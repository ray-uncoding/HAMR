...
path1 = []
path = []
savepath1 = r"C:/Users/dal/Desktop/de_path.csv"
def t(dx,dy,t0,t1,t2,t3,t4,t5,t6):
    ...
    r13 = s5*(c4*(s2*s3*(c0*c1 - s0*s1) - c2*c3*(c0*c1 - s0*s1)) - s4*(c0*s1 + c1*s0)) - c5*(c2*s3*(c0*c1 - s0*s1) + c3*s2*(c0*c1 - s0*s1))
    ...
    r23 = s5*(c4*(s2*s3*(c0*s1 + c1*s0) - c2*c3*(c0*s1 + c1*s0)) + s4*(c0*c1 - s0*s1)) - c5*(c2*s3*(c0*s1 + c1*s0) + c3*s2*(c0*s1 + c1*s0))
    
    r31 = s4*s5*(c2*s3 + c3*s2) - c6*(s5*(c2*c3 - s2*s3) + c4*c5*(c2*s3 + c3*s2))
    r32 = s6*(s5*(c2*c3 - s2*s3) + c4*c5*(c2*s3 + c3*s2)) + c6*s4*(c2*s3 + c3*s2)
    r33 = c4*s5*(c2*s3 + c3*s2) - c5*(c2*c3 - s2*s3)
    
    ex = x - a3*(s2*s3*(c0*c1 - s0*s1) - c2*c3*(c0*c1 - s0*s1)) - d4*(c2*s3*(c0*c1 - s0*s1) + c3*s2*(c0*c1 - s0*s1)) - d6*(c5*(c2*s3*(c0*c1 - s0*s1) + c3*s2*(c0*c1 - s0*s1)) - s5*(c4*(s2*s3*(c0*c1 - s0*s1) - c2*c3*(c0*c1 - s0*s1)) - s4*(c0*s1 + c1*s0))) + a1*(c0*c1 - s0*s1) + a2*c2*(c0*c1 - s0*s1)
    ey = y - a3*(s2*s3*(c0*s1 + c1*s0) - c2*c3*(c0*s1 + c1*s0)) - d4*(c2*s3*(c0*s1 + c1*s0) + c3*s2*(c0*s1 + c1*s0)) - d6*(c5*(c2*s3*(c0*s1 + c1*s0) + c3*s2*(c0*s1 + c1*s0)) - s5*(c4*(s2*s3*(c0*s1 + c1*s0) - c2*c3*(c0*s1 + c1*s0)) + s4*(c0*c1 - s0*s1))) + a1*(c0*s1 + c1*s0) + a2*c2*(c0*s1 + c1*s0)
    ez = d1 - a2*s2 - a3*(c2*s3 + c3*s2) - d4*(c2*c3 - s2*s3) - d6*(c5*(c2*c3 - s2*s3) - c4*s5*(c2*s3 + c3*s2)) + 100
...
#目標座標
# T0 =np.array([[0, 0, -1, -3000], #目標座標
#               [0, -1, 0,-1200],
#               [-1, 0 , 0, 1100],
#               [0, 0, 0, 1]])
T0 =np.array([[-0.0017, 0.276, -0.961, -1600], #目標座標
              [0, -0.961, -0.276,-720],
              [-1, 0 , 0.002, 1020],
              [0, 0, 0, 1]])
# bounds=[(-3300, -900), (-1030, -500),   (0, 0), (m.radians(-2.86), m.radians(90)),\
# (m.radians(-116.75), m.radians(-70)), (m.radians(90), m.radians(130)),\
# (0,0), (m.radians(40),m.radians(125)), (0, 0)]
bounds=[(-3300, -900), (-1400, -500),   (0, 0), (m.radians(-2.86), m.radians(185)),\
(m.radians(-116.75), m.radians(-40)), (m.radians(74.55), m.radians(160)),\
(0, 0), (m.radians(-125),m.radians(125)), (0, 0)]
...
while True:
    ...
    #u是第一代後代   
        ...
        path1 = [newo[0][3],newo[1][3],newo[2][3]]
        path.append(path1)
        qq = LA.norm((q[i] - u))
        # print(qq)
    ...
    if abs(xx) < 1 and abs(yy) < 1 and abs(zz) < 1 :
        ...
        print(t2-t1)
        col = ['x','y','z']
        store = pd.DataFrame(columns = col, data = path)
        store.to_csv(savepath1, encoding = 'utf-8',index = False)
        break
# # x = np.array(x)
# # y = np.array(y)
