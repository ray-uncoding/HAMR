...
//
//vector<Mat> NewtonOptimize::RoutePlanning(int InputCubeX, int InputCubeY, int InputCubeZ, T3DValue point,float SafeThreshold) {
//	
...
//	return Mat();
//}
////Mat NewtonOptimize::OptimizeSinglePos(cv::Mat CurrentPos, cv::Mat StartPos, cv::Mat EndPos) {
////		//cout << filename << endl;
////	
////		//if (fopen_s(&outFile, filename, "w"), outFile != NULL) {
////			cout << "check" << endl;
////			CurrentPos = Normalize(CurrentPos, Forward);
////			StartPos = Normalize(StartPos, Forward);
////			EndPos = Normalize(EndPos, Forward);
////	
////			Mat l = Mat::ones(1, CubeCount, CV_32F);
////			Mat S = Mat::ones(1, CubeCount, CV_32F);
////			Mat u = Mat::ones(CurrentPos.size(), CV_32F);
////			Mat U = Mat::ones(CurrentPos.size(), CV_32F);
////			Mat t = Mat::ones(CurrentPos.size(), CV_32F);
////			Mat V = Mat::ones(CurrentPos.size(), CV_32F);
////	
////			l = l * lagragian_l;
////			S = S * slack_S;
////			u = u * lagragian_u;
...
////			V = V * slack_V;
////	
////			Mat Xm = CurrentPos.t();// starting point?
////			//Mat Xm = Normalize((Mat_<float>(1, 6) << 0, 45, -45, 0, 0, 0), Forward).t();
////			Xm.push_back(l.t());
////			Xm.push_back(S.t());
////			Xm.push_back(u.t());
////			Xm.push_back(U.t());
...
////				float ResultSafetyScoretmp = 0;
////				Mat GradientMatrix = CalcGradient(Xm, CurrentPos, StartPos, EndPos);
////				Mat HessianMatrix = CalcHessian(Xm, CurrentPos, StartPos, EndPos);
////				
////				//Dcount = 0;
////				Mat bla = HessianMatrix.inv();
////				Mat NewIterPos = Xm - HessianMatrix.inv() * GradientMatrix;
////				//cout << HessianMatrix.at<float>(0, 0) << endl;
////				//cout << endl;
////				for (size_t i = 0; i < CubeCount; i++)//Pos Count
////				{
////					ResultSafetyScoretmp += CalcD(i, ExtractSubVector(NewIterPos.t(), TargetVector::Xm));
...
////				ResultSafetyScoretmp = ResultSafetyScoretmp / (float)CubeCount;
////	
////				//auto NewPosXm = ExtractSubVector(NewIterPos.t(), TargetVector::Xm);
...
////				cout << NewPosXm << endl;
////				//if (false) {
////				if (ResultSafetyScoretmp < 0.45f  && abs(ResultSafetyScoretmp - ResultSafetyScore) < 0.001) {
////					//cout << "Final Pos: " << endl;
////					//cout << ResultSafetyScoretmp << endl;
////					//cout << Normalize(CurrentPos, Backward) << endl;
////					//cout << NewPosXm << endl;
////					//cout << i << endl;
////					ResultSafetyScoretmp = 0;
////					ResultSafetyScore = 0;
////					return NewPosXm;
////	
////				}
////	
////				ResultSafetyScore = ResultSafetyScoretmp;
////				PreviousIterPos = Xm.clone();
////				Xm = NewIterPos.clone();
////			}
////		//}
...
//		return result;
//}
//std::vector<std::vector<std::vector<std::vector<float>>>> NewtonOptimize::RobotArm(cv::Mat PathData, int cubeX, int cubeY, int cubeZ, T3DValue point)
//{
//	static const float	r[] = { 112.935,107.26,126.5,84.63,71.77,52.74, };
//	float	    Resolution = sqrt(((RESOLUTION / 2) * (RESOLUTION / 2)) * 3);
//	TArray4X4	tmp, tmpC;
...
//	return output;
//}
//std::vector<float> NewtonOptimize::RobotArmJ(cv::Mat PathData, int cubeX, int cubeY, int cubeZ, T3DValue point)
//{
