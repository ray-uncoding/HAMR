...
void Cylinder::buildVerticesSmooth()
{
    ...
    unsigned int baseVertexIndex = 0; // (unsigned int)vertices.size() / 3;
    ...
    for(int i = 0; i < stackCount; ++i)
    {
        ...
        addIndices(sectorCount, 1, sectorCount+sectorCount+1);
        addIndices(sectorCount+sectorCount+1, 1, sectorCount + 2);
    }
    ...
    for(int i = 0, k = topVertexIndex + 1; i < sectorCount; ++i, ++k)
    {
        if(i < (sectorCount - 1))
            ...
            addIndices(topVertexIndex+1, k, topVertexIndex);
    }
