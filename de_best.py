...
T0 =np.array([[-1836.04], #目標座標
     [-1277.74],
     [-108.1],
     [1]])

bounds=[(-3750, -900), (-680, -480), (m.radians(0), m.radians(0)), (m.radians(-2.86), m.radians(185)),\
(m.radians(-116.75), m.radians(-40)), (m.radians(74.55), m.radians(160)),\
(m.radians(-90), m.radians(90)), (m.radians(-125),m.radians(125)), (m.radians(-1), m.radians(1))]
...
#         qq = LA.norm((q[i]-u))
#         newe = LA.norm((T0 - newo ), ord ='fro') + 0.1*qq + pen*gamma
#         # errr.append(newe)
#         if newe < terr[i]:
#             q[i] = u
#             terr[i] = newe
...
#     # if z == 1000:
#     if abs(xx) < 0.05 and abs(yy) < 0.05 and abs(zz) < 0.05:
#         r = terr.index(min(terr))
